...
(define (root-base a b c math-operation)
  (if (< (square b)
         (* 4 a c)) 
      false
      (/ (math-operation (- b)
...
; *** My own notes ***
; total-displacement = average-velocity * change-in-time
; average-velocity = (initial-velocity + final-velocity) / 2
; final-velocity = initial-velocity + acceleration * change-in-time
; total-displacement = (2*initial-velocity + acceleration * change-in-time)/2 * change-in-time
; total-displacement = (acceleration * change-in-time^2)/2 + initial-velocity * change-in-time
; total-displacement = (- elevation)
; A * ∆t^2 + V * ∆t + elevation = 0
...
(define find-best-angle
  (lambda (velocity elevation)
    (define (find-iter guess best-guess best-displacement)
      (if (> guess 90)
          best-guess
          (let ((displacement (travel-distance-simple elevation velocity guess)))
            (if (> displacement best-displacement)
                (find-iter (+ guess alpha-increment) guess displacement)
                (find-iter (+ guess alpha-increment) best-guess best-displacement)))))
    (find-iter 1 0 0)))



; (find-best-angle 10 0) ; 45
